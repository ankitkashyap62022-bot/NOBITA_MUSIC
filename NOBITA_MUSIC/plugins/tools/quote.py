...
# ==========================================
# ☠️ ANU MATRIX PREMIUM QUOTLY API ☠️
# ==========================================
...
async def get_message_sender_photo(ctx: Message):
    ...
    elif ctx.from_user or ctx.sender_chat and not ctx.sender_chat.photo or not ctx.sender_chat: return ""
    else:
        return {
...
async def pyrogram_to_quotly(messages, is_reply):
    ...
    for message in messages:
        ...
        if message.entities:
            the_message_dict_to_append["entities"] = [{"type": entity.type.name.lower(), "offset": entity.offset, "length": entity.length} for entity in message.entities]
        elif message.caption_entities:
            the_message_dict_to_append["entities"] = [{"type": entity.type.name.lower(), "offset": entity.offset, "length": entity.length} for entity in message.caption_entities]
        else:
...
async def premium_quotly_cmd(client: Client, ctx: Message):
    is_reply = ctx.command[0].endswith("r")
    mystic = await ctx.reply_text("<emoji id=6310044717241340733>🔄</emoji> **Eхᴛʀᴀᴄᴛɪɴɢ Aɴᴜ Mᴀᴛʀɪx Qᴜᴏᴛᴇ...**")
    
    # ☠️ MULTI-MESSAGE QUOTE ☠️
    if len(ctx.text.split()) > 1:
        check_arg = isArgInt(ctx.command[1])
        if check_arg[0]:
            if check_arg[1] < 2 or check_arg[1] > 10:
                return await mystic.edit_text("<emoji id=6307821174017496029>❌</emoji> **Bᴏss, Iɴᴠᴀʟɪᴅ Rᴀɴɢᴇ!**\n<emoji id=5256131095094652290>⏱️</emoji> `Cʜᴏᴏsᴇ ᴀ ɴᴜᴍʙᴇʀ ʙᴇᴛᴡᴇᴇɴ 2 ᴀɴᴅ 10.`")
            try:
                messages = [
                    i for i in await client.get_messages(
                        chat_id=ctx.chat.id,
                        message_ids=range(ctx.reply_to_message.id, ctx.reply_to_message.id + check_arg[1]),
                        replies=-1,
                    ) if not i.empty and not i.media
                ]
            except Exception as e:
                return await mystic.edit_text(f"<emoji id=6307821174017496029>❌</emoji> **Mᴇssᴀɢᴇ Eхᴛʀᴀᴄᴛɪᴏɴ Fᴀɪʟᴇᴅ!**\n`{e}`")
                
            try:
                await mystic.edit_text("<emoji id=6123040393769521180>☄️</emoji> **Gᴇɴᴇʀᴀᴛɪɴɢ Sᴛɪᴄᴋᴇʀ...**")
                make_quotly = await pyrogram_to_quotly(messages, is_reply=is_reply)
                bio_sticker = BytesIO(make_quotly)
                bio_sticker.name = "anu_matrix_quote.webp" # ☠️ Cringe Name Removed
                await ctx.reply_sticker(bio_sticker)
                return await mystic.delete()
            except Exception as e:
                return await mystic.edit_text(f"<emoji id=6307821174017496029>❌</emoji> **API Eʀʀᴏʀ:** `{e}`")

    # ☠️ SINGLE MESSAGE QUOTE ☠️
    try:
        messages_one = await client.get_messages(chat_id=ctx.chat.id, message_ids=ctx.reply_to_message.id, replies=-1)
